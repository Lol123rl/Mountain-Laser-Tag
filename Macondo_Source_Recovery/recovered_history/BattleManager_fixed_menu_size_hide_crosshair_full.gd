...
func apply_responsive_title_ui() -> void:
	...
	position_title_button(title_reset_progress_button, button_width, button_height, start_y + 2.0 * (button_height + gap), scale)
	position_title_button(title_brief_button, button_width, button_height, start_y + 3.0 * (button_height + gap), scale)
	position_title_button(title_mode_button, button_width, button_height, start_y + 4.0 * (button_height + gap), scale)
	position_title_button(title_quit_button, button_width, button_height, start_y + 5.0 * (button_height + gap), scale)
...
func create_title_screen() -> void:
	...
	title_reset_progress_button = make_title_button("RESET SAVE", Vector2(-165.0, 46.0), Vector2(330.0, 44.0))
	...
	title_brief_button = make_title_button("MISSION BRIEF", Vector2(-165.0, 96.0), Vector2(330.0, 44.0))
	title_brief_button.pressed.connect(toggle_title_brief)
	title_panel.add_child(title_brief_button)

	title_mode_button = make_title_button(get_mode_button_text(), Vector2(-165.0, 146.0), Vector2(330.0, 44.0))
	title_mode_button.pressed.connect(cycle_game_mode)
	title_panel.add_child(title_mode_button)

	title_quit_button = make_title_button("QUIT", Vector2(-165.0, 196.0), Vector2(330.0, 44.0))
...
func get_mode_button_text() -> String:
	return "MODE: " + get_game_mode_display_name(selected_game_mode)
...
func cycle_game_mode() -> void:
	var current_index: int = GAME_MODE_LIST.find(selected_game_mode)
	if current_index < 0:
		current_index = 0
	current_index = (current_index + 1) % GAME_MODE_LIST.size()
	selected_game_mode = GAME_MODE_LIST[current_index]
