...
class MiniMapPanel:
	...
	const REDRAW_INTERVAL: float = 0.10 # 10 minimap redraws per second is smoother than redrawing every frame.
...
class MountainTitlePanel:
	...
	func _draw() -> void:
		...
		# It is taller now so the buttons and mission brief stay inside the box.
		var card_size: Vector2 = Vector2(min(860.0, s.x * 0.86), min(580.0, s.y * 0.84))
		var card_pos: Vector2 = Vector2((s.x - card_size.x) * 0.5, s.y * 0.075)
...
const SMOOTH_MAX_ACTIVE_UNITS_PER_TEAM: int = 6
const BATTLE_UI_UPDATE_INTERVAL: float = 0.12
...
const NPC_SEARCH_REPICK_TIME_MIN: float = 3.5
const NPC_SEARCH_REPICK_TIME_MAX: float = 8.0
...
const NPC_REPATH_TIME: float = 1.9
...
# ---------------- DISPLAY / FULLSCREEN ----------------
const AUTO_FULLSCREEN_ON_START: bool = true
const AUTO_SET_PROJECT_DISPLAY: bool = true
const TARGET_VIEWPORT_WIDTH: int = 1280
const TARGET_VIEWPORT_HEIGHT: int = 720
const START_FULLSCREEN: bool = true
const RESPONSIVE_UI_ENABLED: bool = true
const UI_REFERENCE_WIDTH: float = 1152.0
const UI_REFERENCE_HEIGHT: float = 648.0
const UI_MIN_SCALE: float = 0.70
const UI_MAX_SCALE: float = 1.00
...
const HILL_SCORE_TO_WIN: float = 100.0
...
const FINAL_BOSS_HEALTH: int = 40
const FINAL_BOSS_SIZE_SCALE: Vector3 = Vector3(1.55, 1.55, 1.55)
const FINAL_BOSS_CENTER: Vector3 = Vector3(0.0, 0.0, 0.0)
const FINAL_BOSS_SHOWDOWN_RADIUS: float = 115.0
const FINAL_BOSS_RED_FIRE_MULTIPLIER: float = 0.72
...
const TITLE_MUSIC_VOLUME_DB: float = -7.0
...
const RED_ACTIVE_LIMIT: int = 6
const BLUE_ACTIVE_LIMIT: int = 6

const RED_TOTAL_RESERVES: int = 40
const BLUE_TOTAL_RESERVES: int = 40
...
const NPC_HEADSHOT_DAMAGE: int = 999
...
const HELICOPTER_STRATEGY_CHECK_TIME: float = 1.0
...
var achievement_data: Dictionary = {
	...
	"ten_headshots_match": {
		"title": "Sharpshooter",
		"desc": "Get 10 headshots in one match.",
		"hidden": true
	},
	"flag_capture": {
		"title": "Flag Runner",
		"desc": "Capture one enemy flag.",
		"hidden": true
	},
	"manhunt_survivor": {
		"title": "Ghost in the Fog",
		"desc": "Survive a Manhunt match as the hunted player.",
		"hidden": true
	},
	"hill_holder": {
		"title": "Hold the High Ground",
		"desc": "Hold the hill long enough to earn a hill-control reward.",
		"hidden": true
	},
	"helicopter_ace": {
		"title": "Rotor Ace",
		"desc": "Get 5 helicopter eliminations in one match.",
		"hidden": true
	},
	"commander_breaker": {
		"title": "Command Breaker",
		"desc": "Win Commander mode.",
		"hidden": true
	},
	"team_elim_win": {
		"title": "Linebreaker",
		"desc": "Win Team Elimination.",
		"hidden": true
	},
	"boss_unlocked": {
		"title": "The Gate Opens",
		"desc": "Unlock the final showdown.",
		"hidden": true
	},
	"final_boss": {
...
func _physics_process(delta: float) -> void:
	update_responsive_ui_if_needed()
...
func setup_display_and_window() -> void:
	apply_project_display_settings()

	# Let UI anchors resize correctly when the window/screen changes.
	if not get_viewport().size_changed.is_connected(update_responsive_ui_if_needed):
		get_viewport().size_changed.connect(update_responsive_ui_if_needed)

	call_deferred("update_responsive_ui_if_needed")


func apply_project_display_settings() -> void:
	if not AUTO_SET_PROJECT_DISPLAY:
		if AUTO_FULLSCREEN_ON_START:
			DisplayServer.window_set_mode(DisplayServer.WINDOW_MODE_FULLSCREEN)
		return

	# Runtime equivalent of Project Settings:
	# Project -> Project Settings -> Display -> Window
	ProjectSettings.set_setting("display/window/size/viewport_width", TARGET_VIEWPORT_WIDTH)
	ProjectSettings.set_setting("display/window/size/viewport_height", TARGET_VIEWPORT_HEIGHT)
	ProjectSettings.set_setting("display/window/stretch/mode", "canvas_items")
	ProjectSettings.set_setting("display/window/stretch/aspect", "expand")

	var target_size: Vector2i = Vector2i(TARGET_VIEWPORT_WIDTH, TARGET_VIEWPORT_HEIGHT)
	...
	# Force the viewport to recalculate UI layout after the display change.
	call_deferred("update_responsive_ui_if_needed")
...
func get_ui_scale_factor() -> float:
	...
	var scale_x: float = viewport_size.x / UI_REFERENCE_WIDTH
	var scale_y: float = viewport_size.y / UI_REFERENCE_HEIGHT
	return clamp(min(scale_x, scale_y), UI_MIN_SCALE, UI_MAX_SCALE)


func update_responsive_ui_if_needed() -> void:
	if not RESPONSIVE_UI_ENABLED:
		return
	...
	current_ui_scale = get_ui_scale_factor()

	apply_responsive_title_ui()
	apply_responsive_shop_ui()
	apply_responsive_battle_ui()


func apply_responsive_title_ui() -> void:
	if title_panel == null:
		return

	var s: Vector2 = get_viewport().get_visible_rect().size
	var scale: float = current_ui_scale

	# Keep the title inside the screen and stop it from growing too large.
	if title_label != null:
		title_label.offset_top = 30.0
		title_label.offset_bottom = 100.0
		title_label.add_theme_font_size_override("font_size", 48)

	if title_subtitle_label != null:
		title_subtitle_label.offset_top = 112.0
		title_subtitle_label.offset_bottom = 148.0
		title_subtitle_label.add_theme_font_size_override("font_size", 22)

	if title_hint_label != null:
		title_hint_label.offset_top = 165.0
		title_hint_label.offset_bottom = 200.0
		title_hint_label.add_theme_font_size_override("font_size", 15)

	# Fixed 16:9-safe button stack. This fits 1280x720 and larger.
	var button_width: float = clamp(s.x * 0.31, 300.0, 390.0)
	var button_height: float = 38.0
	var gap: float = 6.0
	var start_y: float = -82.0

	position_title_button(title_start_button, button_width, button_height, start_y + 0.0 * (button_height + gap), scale)
	position_title_button(title_shop_button, button_width, button_height, start_y + 1.0 * (button_height + gap), scale)
	position_title_button(title_achievements_button, button_width, button_height, start_y + 2.0 * (button_height + gap), scale)
	position_title_button(title_reset_progress_button, button_width, button_height, start_y + 3.0 * (button_height + gap), scale)
	position_title_button(title_brief_button, button_width, button_height, start_y + 4.0 * (button_height + gap), scale)
	position_title_button(title_mode_button, button_width, button_height, start_y + 5.0 * (button_height + gap), scale)
	position_title_button(title_quit_button, button_width, button_height, start_y + 6.0 * (button_height + gap), scale)

	if title_brief_label != null:
		title_brief_label.offset_left = -min(360.0, s.x * 0.42)
		title_brief_label.offset_right = min(360.0, s.x * 0.42)
		title_brief_label.offset_top = 230.0
		title_brief_label.offset_bottom = 330.0
		title_brief_label.add_theme_font_size_override("font_size", 14)

func position_title_button(button: Button, width: float, height: float, top: float, scale: float) -> void:
	if button == null:
		return
...
func apply_responsive_shop_ui() -> void:
	if shop_root == null or main_panel == null:
		return

	var s: Vector2 = get_viewport().get_visible_rect().size
	var scale: float = current_ui_scale

	# Large screens: margins. Small screens: nearly full screen.
	main_panel.anchor_left = 0.035
	main_panel.anchor_right = 0.965
	main_panel.anchor_top = 0.025
	main_panel.anchor_bottom = 0.975
	...
	if money_label != null:
		money_label.add_theme_font_size_override("font_size", int(clamp(22.0 * scale, 17.0, 28.0)))

	if detail_label != null:
		detail_label.add_theme_font_size_override("font_size", int(clamp(15.0 * scale, 12.0, 19.0)))

	if status_label != null:
		status_label.add_theme_font_size_override("font_size", int(clamp(14.0 * scale, 11.0, 17.0)))

	for upgrade_id in upgrade_buttons.keys():
		var button: Button = upgrade_buttons[upgrade_id]
		if button != null:
			button.custom_minimum_size = Vector2(0.0, clamp(48.0 * scale, 38.0, 62.0))
			button.add_theme_font_size_override("font_size", int(clamp(15.0 * scale, 12.0, 19.0)))

	if close_button != null:
		close_button.custom_minimum_size = Vector2(0.0, clamp(34.0 * scale, 28.0, 44.0))

	if save_button != null:
		save_button.custom_minimum_size = Vector2(0.0, clamp(34.0 * scale, 28.0, 44.0))

	if reset_progress_button != null:
		reset_progress_button.custom_minimum_size = Vector2(0.0, clamp(34.0 * scale, 28.0, 44.0))

	if confirm_panel != null:
		var confirm_width: float = clamp(s.x * 0.45, 360.0, 560.0)
		var confirm_height: float = clamp(s.y * 0.36, 230.0, 320.0)
		confirm_panel.anchor_left = 0.5
		confirm_panel.anchor_right = 0.5
		confirm_panel.anchor_top = 0.5
		confirm_panel.anchor_bottom = 0.5
		confirm_panel.offset_left = -confirm_width * 0.5
		confirm_panel.offset_right = confirm_width * 0.5
		confirm_panel.offset_top = -confirm_height * 0.5
		confirm_panel.offset_bottom = confirm_height * 0.5


func apply_responsive_battle_ui() -> void:
	var scale: float = current_ui_scale

	if battle_label != null:
		battle_label.position = Vector2(0.0, 8.0 * scale)
		battle_label.add_theme_font_size_override("font_size", int(clamp(24.0 * scale, 18.0, 32.0)))

	if result_label != null:
		result_label.add_theme_font_size_override("font_size", int(clamp(34.0 * scale, 24.0, 46.0)))

	if result_menu_button != null:
		var width: float = clamp(280.0 * scale, 220.0, 360.0)
		var height: float = clamp(56.0 * scale, 42.0, 70.0)
		result_menu_button.offset_left = -width * 0.5
		result_menu_button.offset_right = width * 0.5
		result_menu_button.offset_top = 150.0 * scale
		result_menu_button.offset_bottom = 150.0 * scale + height
		result_menu_button.add_theme_font_size_override("font_size", int(clamp(22.0 * scale, 16.0, 30.0)))
...
func update_early_quit_input() -> void:
	...
	var quit_key_down: bool = Input.is_physical_key_pressed(KEY_ESCAPE) or Input.is_physical_key_pressed(KEY_Q)
...
func update_manhunt_survival_rewards(delta: float) -> void:
	...
	if manhunt_survival_reward_timer >= MANHUNT_REWARD_INTERVAL_SECONDS:
		...
		award_money("Manhunt survival", MONEY_MANHUNT_SURVIVE_TICK)
...
func create_battle_ui() -> void:
	...
	result_panel.color = Color(0.0, 0.0, 0.0, 0.76)
	...
	result_menu_button.offset_left = -140.0
	result_menu_button.offset_right = 140.0
	result_menu_button.offset_top = 160.0
	result_menu_button.offset_bottom = 216.0
	result_menu_button.visible = false
	result_menu_button.z_index = 10
	result_menu_button.add_theme_font_size_override("font_size", 24)
...
func create_title_screen() -> void:
	...
	title_label.offset_top = 64.0
	title_label.offset_bottom = 148.0
	...
	title_subtitle_label.offset_top = 145.0
	title_subtitle_label.offset_bottom = 190.0
	...
	title_subtitle_label.add_theme_font_size_override("font_size", 24)
	...
	title_hint_label.offset_top = 218.0
	title_hint_label.offset_bottom = 268.0
	...
	title_hint_label.add_theme_font_size_override("font_size", 19)
	...
	title_start_button = make_title_button("DEPLOY", Vector2(-165.0, -58.0), Vector2(330.0, 52.0))
	title_start_button.pressed.connect(start_game_from_title)
	title_panel.add_child(title_start_button)

	title_shop_button = make_title_button("UPGRADE SHOP", Vector2(-165.0, -6.0), Vector2(330.0, 46.0))
	title_shop_button.pressed.connect(open_shop_from_title)
	title_panel.add_child(title_shop_button)

	title_achievements_button = make_title_button("ACHIEVEMENTS", Vector2(-165.0, 46.0), Vector2(330.0, 44.0))
	title_achievements_button.pressed.connect(open_achievements_from_title)
	title_panel.add_child(title_achievements_button)

	title_reset_progress_button = make_title_button("RESET SAVE", Vector2(-165.0, 96.0), Vector2(330.0, 44.0))
	...
	title_brief_button = make_title_button("MISSION BRIEF", Vector2(-165.0, 146.0), Vector2(330.0, 44.0))
	title_brief_button.pressed.connect(toggle_title_brief)
	title_panel.add_child(title_brief_button)

	title_mode_button = make_title_button(get_mode_button_text(), Vector2(-165.0, 196.0), Vector2(330.0, 44.0))
	title_mode_button.pressed.connect(cycle_game_mode)
	title_panel.add_child(title_mode_button)

	title_quit_button = make_title_button("QUIT", Vector2(-165.0, 246.0), Vector2(330.0, 44.0))
	...
	title_brief_label.offset_left = -350.0
	title_brief_label.offset_right = 350.0
	title_brief_label.offset_top = 176.0
	title_brief_label.offset_bottom = 286.0
	...
	title_brief_label.add_theme_font_size_override("font_size", 16)
...
func create_online_menu_controls() -> void:
	...
	online_host_public_button.add_theme_font_size_override("font_size", 24)
...
func get_title_brief_text() -> String:
	if selected_game_mode == GAME_MODE_CTF:
		return "MISSION BRIEF\nCapture the enemy flag and bring it back to your base.\nFirst team to 2 captures wins. Respawns are infinite.\nMini map: green = friendlies, red = enemies, white = you, flags = colored diamonds."
	elif selected_game_mode == GAME_MODE_MANHUNT:
		return "MISSION BRIEF\nManhunt is now a duel: you versus one Red CPU. 50% chance you are hunted; 50% chance Red is hunted. No blue NPC teammates. No red squad. No respawns."
	elif selected_game_mode == GAME_MODE_COMMANDER:
		return "MISSION BRIEF\nEach team has a commander. Knock out the enemy commander or eliminate the whole enemy team to win. Protect your commander and push together."
	elif selected_game_mode == GAME_MODE_KING_HILL:
		return "MISSION BRIEF\nControl the mountain relay point. Stand inside the glowing hill zone to score. First team to 100 points wins, or highest score when time expires."
	return "MISSION BRIEF\nKnock out the Red Team before the timer expires.\nMini map: green = friendlies, red = enemies, white = you.\nUse cover, watch the ridges, and take the helicopter when safe."
...
func start_game_from_title() -> void:
	...
	if minimap_panel:
		# Manhunt is better without radar. The hunted player should be able to hide.
...
func get_blue_total_left() -> int:
	if selected_game_mode == GAME_MODE_CTF:
		var player_ctf_alive: int = 0 if is_player_dead() else 1
		return blue_units.size() + player_ctf_alive

	# Player knock-outs now cost Blue one ticket. This makes the Blue counter drop
	# when the player dies instead of only tracking Blue CPU soldiers.
	var blue_left_to_spawn: int = max(BLUE_TOTAL_RESERVES - blue_spawned_total - player_deaths, 0)
	return blue_units.size() + blue_left_to_spawn
...
func show_result_screen(result: String, extra_line: String = "") -> void:
	...
	if result_label:
		...
		var mode_line: String = "Mode: " + get_game_mode_display_name(selected_game_mode)
		var score_line: String = ""
		if selected_game_mode == GAME_MODE_CTF:
			score_line = "\nBlue Captures: " + str(blue_ctf_score) + " | Red Captures: " + str(red_ctf_score)
		elif selected_game_mode == GAME_MODE_KING_HILL:
			score_line = "\nBlue Hill Score: " + str(int(blue_hill_score)) + " | Red Hill Score: " + str(int(red_hill_score))
		elif selected_game_mode == GAME_MODE_MANHUNT:
			score_line = "\nHunted Team: " + manhunt_hunted_team.capitalize()
		elif selected_game_mode == GAME_MODE_COMMANDER:
			score_line = "\n" + get_commander_status_text()
		else:
			score_line = "\nPlayer Knock Outs: " + str(player_kills) + "\nInstant Knock Outs: " + str(player_headshots) + "\nPlayer Times Knocked Out: " + str(player_deaths)
...
func update_manhunt_mode(_delta: float) -> void:
	...
	if manhunt_hunted_target == null or not is_instance_valid(manhunt_hunted_target):
		if manhunt_hunted_team == "red":
			...
			show_result_screen("DEFEAT - HUNTED TARGET LOST")
		return
...
func update_respawns(delta: float) -> void:
	...
	if red_units.size() < RED_ACTIVE_LIMIT and (uses_infinite_respawns() or red_spawned_total < RED_TOTAL_RESERVES) and red_respawn_timer <= 0.0:
		spawn_one_unit("red")
		red_respawn_timer = rng.randf_range(RESPAWN_DELAY_MIN, RESPAWN_DELAY_MAX)

	if blue_units.size() < get_blue_npc_active_limit() and (uses_infinite_respawns() or blue_spawned_total < BLUE_TOTAL_RESERVES) and blue_respawn_timer <= 0.0:
		spawn_one_unit("blue")
...
func spawn_one_unit(team: String) -> void:
	...
	if team == "red":
		if not uses_infinite_respawns() and red_spawned_total >= RED_TOTAL_RESERVES:
			return
		red_spawned_total += 1
	else:
...
func update_bullets(delta: float) -> void:
	...
	for i in range(bullets.size() - 1, -1, -1):
		...
		var direction: Vector3 = bullet.get_meta("direction") as Vector3
		...
		if hit.size() > 0:
			...
			handle_bullet_hit(bullet_team, hit_object, hit_position, hit_normal)
		...
		if bullet_segment_hits_player(old_position, new_position, bullet_team):
			apply_red_bullet_damage_to_player(new_position)
			bullets.remove_at(i)
			bullet.queue_free()
			continue
...
func bullet_segment_hits_player(old_position: Vector3, new_position: Vector3, bullet_team: String) -> bool:
	if bullet_team != "red":
		return false
	...
	var closest_point: Vector3 = closest_point_on_segment(player.global_position + Vector3(0.0, 0.9, 0.0), old_position, new_position)
	var distance_to_player: float = closest_point.distance_to(player.global_position + Vector3(0.0, 0.9, 0.0))

	return distance_to_player <= PLAYER_BULLET_HIT_RADIUS
...
func apply_red_bullet_damage_to_player(hit_position: Vector3) -> void:
	if player == null or not is_instance_valid(player) or is_player_dead():
		return
	...
	if player.has_method("take_damage"):
		player.call("take_damage", NPC_BULLET_DAMAGE)
...
func handle_bullet_hit(bullet_team: String, hit_object: Node, hit_position: Vector3, hit_normal: Vector3) -> void:
	...
	if hit_player != null:
		if bullet_team == "red":
			...
			if hit_player.has_method("take_damage"):
				hit_player.call("take_damage", NPC_BULLET_DAMAGE)
...
func get_meta_vector3(unit: CharacterBody3D, key: String, fallback: Vector3) -> Vector3:
	if unit == null:
		return fallback
	...
	return unit.get_meta(key) as Vector3


# ---------------- SHOP / ECONOMY IS INTEGRATED BELOW ----------------
...
func record_elimination(headshot: bool, from_helicopter: bool = false) -> void:
	var reward: int = MONEY_PER_ELIMINATION

	if headshot:
		reward += MONEY_HEADSHOT_BONUS
	...
	save_shop_data()
	show_status("Earned $" + str(reward) + " from elimination.")
	update_shop_text()
...
func build_shop_ui() -> void:
	...
	reset_progress_button.add_theme_font_size_override("font_size", 16)
...
func build_confirm_panel() -> void:
	...
	confirm_yes_button.add_theme_font_size_override("font_size", 16)
	...
	confirm_no_button.add_theme_font_size_override("font_size", 16)
...
func reset_progress_confirmed() -> void:
	...
	apply_all_upgrades()
	update_shop_text()
	show_status("Progress reset. Money and upgrades are now zero.")
...
func update_achievement_unlocks() -> void:
	...
	if are_all_normal_achievements_unlocked():
		if not boss_mode_unlocked:
			boss_mode_unlocked = true
			set_meta(BOSS_MODE_UNLOCKED_META, true)
			unlock_achievement("boss_unlocked")
	else:
		boss_mode_unlocked = false
		set_meta(BOSS_MODE_UNLOCKED_META, false)

	update_title_mode_button_text()
...
func is_final_boss_unlocked() -> bool:
	setup_achievement_defaults()
	return bool(achievement_unlocked.get("boss_unlocked", false)) or are_all_normal_achievements_unlocked()
...
func build_achievement_ui() -> void:
	...
	achievement_panel.anchor_left = 0.08
	achievement_panel.anchor_right = 0.92
	achievement_panel.anchor_top = 0.08
	achievement_panel.anchor_bottom = 0.92
	...
	achievement_panel.add_theme_stylebox_override("panel", make_panel_style(Color(0.010, 0.014, 0.022, 0.97), Color(0.80, 0.68, 0.24, 0.95), 3))
	...
	achievement_list_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	achievement_list_label.size_flags_vertical = Control.SIZE_EXPAND_FILL
	achievement_list_label.add_theme_font_size_override("font_size", 17)
	achievement_list_label.add_theme_color_override("font_color", Color(0.90, 0.94, 1.0, 1.0))
	box.add_child(achievement_list_label)

	achievement_close_button = Button.new()
	achievement_close_button.text = "BACK"
	achievement_close_button.custom_minimum_size = Vector2(0.0, 42.0)
	achievement_close_button.add_theme_font_size_override("font_size", 18)
...
func update_achievement_page() -> void:
	...
	for achievement_id in achievement_data.keys():
		...
		if unlocked:
			lines.append("✓ " + str(data["title"]) + " — " + str(data["desc"]))
		else:
			if bool(data.get("hidden", false)):
				lines.append("□ ??? — Hidden achievement")
			else:
				lines.append("□ " + str(data["title"]) + " — " + str(data["desc"]))

	lines.insert(0, "Unlocked: " + str(unlocked_count) + "/" + str(normal_count) + "\n")

	if is_final_boss_unlocked() and not bool(achievement_unlocked.get("final_boss", false)):
		lines.append("\nFINAL MODE UNLOCKED: The last achievement waits in the showdown.")

	achievement_list_label.text = "\n".join(lines)
...
# ---------------- INTEGRATED SUPPLY CRATE SYSTEM ----------------
# To make a crate without a separate script:
# 1. Add any Node3D/MeshInstance3D to group "SupplyCrate".
# 2. Add metadata named "crate_type" with one of:
#    "health", "ammo", "stamina", "fuel", "money".
# 3. BattleManager will handle pickup, effect, hiding, and timed respawn.
