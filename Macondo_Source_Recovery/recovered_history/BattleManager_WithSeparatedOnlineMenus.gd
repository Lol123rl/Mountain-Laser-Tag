...
class MountainTitlePanel:
	...
	func _draw() -> void:
		...
		var card_size: Vector2 = Vector2(min(780.0, s.x * 0.82), min(540.0, s.y * 0.82))
		var card_pos: Vector2 = Vector2((s.x - card_size.x) * 0.5, s.y * 0.10)
...
const RED_ACTIVE_LIMIT: int = 9
const BLUE_ACTIVE_LIMIT: int = 9
...
func online_is_server_or_single_player() -> bool:
	return not online_is_active() or multiplayer.is_server()
...
func create_title_screen() -> void:
	...
	title_subtitle_label.text = "Night battle in the Mountains  |  Blue Team vs Red Team"
	...
	title_hint_label.text = "WASD move  |  Mouse aim  |  Left click fire  |  E helicopter  |  M change mode"
	title_hint_label.z_index = 502
	title_panel.add_child(title_hint_label)

	title_start_button = make_title_button("SOLO GAME", Vector2(-150.0, -30.0), Vector2(300.0, 50.0))
	title_start_button.pressed.connect(online_start_button_pressed)
	title_panel.add_child(title_start_button)

	online_open_button = make_title_button("ONLINE", Vector2(-150.0, 28.0), Vector2(300.0, 46.0))
	online_open_button.pressed.connect(show_online_menu)
	title_panel.add_child(online_open_button)

	title_brief_button = make_title_button("MISSION BRIEF", Vector2(-150.0, 82.0), Vector2(300.0, 46.0))
	title_brief_button.pressed.connect(toggle_title_brief)
	title_panel.add_child(title_brief_button)

	title_mode_button = make_title_button(get_mode_button_text(), Vector2(-150.0, 136.0), Vector2(300.0, 46.0))
	title_mode_button.pressed.connect(cycle_game_mode)
	title_panel.add_child(title_mode_button)

	title_quit_button = make_title_button("QUIT", Vector2(-150.0, 190.0), Vector2(300.0, 46.0))
...
func create_online_menu_controls() -> void:
	...
	online_panel.anchor_left = 0.5
	online_panel.anchor_right = 0.5
	online_panel.anchor_top = 0.5
	online_panel.anchor_bottom = 0.5
	# Centered inside the title card so it does not run off screen.
	online_panel.offset_left = -345.0
	online_panel.offset_right = 345.0
	online_panel.offset_top = -92.0
	online_panel.offset_bottom = 286.0
	online_panel.z_index = 508
	...
	online_box.add_theme_constant_override("separation", 7)
	online_panel.add_child(online_box)

	online_lobby_title_label = Label.new()
	online_lobby_title_label.text = "ONLINE LOBBY"
	online_lobby_title_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	online_lobby_title_label.add_theme_font_size_override("font_size", 26)
	online_box.add_child(online_lobby_title_label)

	online_lobby_state_label = Label.new()
	online_lobby_state_label.text = "Default: Solo Game. Open this menu only when you want online play."
	...
	online_lobby_state_label.add_theme_font_size_override("font_size", 15)
	...
	online_party_count_label.add_theme_font_size_override("font_size", 18)
	online_box.add_child(online_party_count_label)

	online_private_code_label = Label.new()
	online_private_code_label.text = "Private Code: ---"
	online_private_code_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	online_private_code_label.add_theme_font_size_override("font_size", 18)
	online_box.add_child(online_private_code_label)

	var server_label: Label = Label.new()
	server_label.text = "Dedicated Server IP"
	server_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	online_box.add_child(server_label)

	online_server_ip_line_edit = LineEdit.new()
	online_server_ip_line_edit.name = "ServerIpLineEdit"
	online_server_ip_line_edit.placeholder_text = "Example: 127.0.0.1"
	online_server_ip_line_edit.text = "127.0.0.1"
	online_box.add_child(online_server_ip_line_edit)

	var host_row: HBoxContainer = HBoxContainer.new()
	host_row.name = "HostRow"
	host_row.add_theme_constant_override("separation", 8)
	online_box.add_child(host_row)

	online_host_public_button = Button.new()
	online_host_public_button.text = "HOST PUBLIC"
	online_host_public_button.custom_minimum_size = Vector2(190.0, 38.0)
	online_host_public_button.pressed.connect(online_host_public_pressed)
	host_row.add_child(online_host_public_button)

	online_host_private_button = Button.new()
	online_host_private_button.text = "HOST PRIVATE"
	online_host_private_button.custom_minimum_size = Vector2(190.0, 38.0)
	online_host_private_button.pressed.connect(online_host_private_pressed)
	host_row.add_child(online_host_private_button)

	var join_row: HBoxContainer = HBoxContainer.new()
	join_row.name = "JoinPrivateRow"
	join_row.add_theme_constant_override("separation", 8)
	online_box.add_child(join_row)

	online_join_code_line_edit = LineEdit.new()
	online_join_code_line_edit.name = "JoinCodeLineEdit"
	online_join_code_line_edit.placeholder_text = "Private code"
	online_join_code_line_edit.custom_minimum_size = Vector2(190.0, 38.0)
	join_row.add_child(online_join_code_line_edit)

	online_join_private_button = Button.new()
	online_join_private_button.text = "JOIN PRIVATE"
	online_join_private_button.custom_minimum_size = Vector2(190.0, 38.0)
	online_join_private_button.pressed.connect(online_join_private_pressed)
	join_row.add_child(online_join_private_button)

	var public_row: HBoxContainer = HBoxContainer.new()
	public_row.name = "PublicRow"
	public_row.add_theme_constant_override("separation", 8)
	online_box.add_child(public_row)

	online_refresh_public_button = Button.new()
	online_refresh_public_button.text = "REFRESH PUBLIC"
	online_refresh_public_button.custom_minimum_size = Vector2(190.0, 38.0)
	...
	online_join_selected_public_button.text = "JOIN SELECTED"
	online_join_selected_public_button.custom_minimum_size = Vector2(190.0, 38.0)
	...
	online_public_rooms_item_list.custom_minimum_size = Vector2(400.0, 62.0)
	...
	online_status_label.text = "Status: Solo mode. No online room joined."
	...
	online_status_label.add_theme_font_size_override("font_size", 14)
	online_box.add_child(online_status_label)

	online_back_button = Button.new()
	online_back_button.text = "BACK TO MAIN MENU"
	online_back_button.custom_minimum_size = Vector2(400.0, 38.0)
...
func online_update_lobby_labels() -> void:
	...
	if online_private_code_label != null:
		if online_current_room_private and not online_current_join_code.is_empty():
			online_private_code_label.text = "Private Code: " + online_current_join_code
		else:
			online_private_code_label.text = "Private Code: ---"
	...
	if online_lobby_state_label != null:
		if multiplayer.multiplayer_peer == null:
			...
			online_lobby_state_label.text = "CONNECTED. Host a room or join a room."
		elif online_current_room_private:
			online_lobby_state_label.text = "HOSTING PRIVATE ROOM" if online_current_room_host_id == multiplayer.get_unique_id() else "JOINED PRIVATE ROOM"
		else:
...
func online_ensure_connected() -> void:
	if multiplayer.multiplayer_peer != null:
		return

	if online_server_ip_line_edit == null:
		online_set_status("Online menu is not ready.")
		return

	var server_ip: String = online_server_ip_line_edit.text.strip_edges()
	if server_ip.is_empty():
		online_set_status("Enter a dedicated server IP first.")
		return

	online_set_status("Connecting to server...")
	online_connect_to_dedicated_server(server_ip, ONLINE_DEFAULT_PORT)


func online_host_public_pressed() -> void:
	online_ensure_connected()
...
func online_client_room_created(room_id: String, join_code: String, private_room: bool) -> void:
	...
	online_current_party_max = ONLINE_MAX_ROOM_PLAYERS
	if private_room:
		online_set_status("Hosting private room. Share code: " + join_code)
	else:
		online_set_status("Hosting public room. Waiting for players.")
...
func online_client_public_rooms(public_rooms: Array) -> void:
	...
	for room in public_rooms:
		...
		online_public_rooms_item_list.add_item(str(data["mode"]).to_upper() + "    Party " + str(data["players"]) + "/" + str(data["max_players"]))
...
func get_title_brief_text() -> String:
	if selected_game_mode == GAME_MODE_CTF:
		...
		return "MISSION BRIEF\nOne side has one hunted player. The hunted side must hide and survive until time expires. The hunters win if they knock out the hunted target. 50% chance you are hunted. The hunted target cannot use helicopters."
...
func start_game_from_title() -> void:
	if online_is_active() and not multiplayer.is_server() and not online_starting_from_server:
		request_start_game_from_server.rpc_id(1, selected_game_mode)
		return
...
func start_match() -> void:
	# Clients receive spawned/synchronized objects from the server.
	if online_is_active() and not multiplayer.is_server():
		find_player()
		return
	...
	for i in range(RED_ACTIVE_LIMIT):
		spawn_one_unit("red")

	for i in range(get_blue_npc_active_limit()):
		spawn_one_unit("blue")
...
func draw_minimap_content(canvas: Control) -> void:
	...
	# Player = white dot.
	if player != null and is_instance_valid(player):
		draw_minimap_position(canvas, player.global_position, Color(1.0, 1.0, 1.0, 1.0), MINIMAP_PLAYER_DOT_SIZE)
...
func record_player_kill(headshot: bool) -> void:
	player_kills += 1
	blue_kills += 1
...
func show_result_screen(result: String, extra_line: String = "") -> void:
	...
	if result_label:
		...
		result_label.text = (
			...
			extra_text +
			score_line +
			"\n\nPress ENTER, SPACE, or R for main menu"
		)
...
func uses_infinite_respawns() -> bool:
	return selected_game_mode == GAME_MODE_CTF or selected_game_mode == GAME_MODE_KING_HILL or selected_game_mode == GAME_MODE_MANHUNT
...
func create_unit(team: String, index: int) -> CharacterBody3D:
	...
	unit.set_meta("fire_cooldown", rng.randf_range(NPC_FIRE_COOLDOWN_MIN, NPC_FIRE_COOLDOWN_MAX))
