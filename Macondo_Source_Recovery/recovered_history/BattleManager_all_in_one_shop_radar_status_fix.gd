...
# Lower active NPC count if the game stutters. 6 per side is a good first test.
...
const RADAR_BONUS_SECONDS_PER_LEVEL: float = 1.0
const PLAYER_RADAR_PING_BASE_TIME: float = 2.0

var player_radar_ping_timer: float = 0.0
...
var upgrade_data: Dictionary = {
	...
	"radar": {
		"title": "Recon Scanner",
		"short": "Radar pings last longer.",
		"effect": "+1 second radar ping per level",
		"max_level": 5,
		"base_cost": 200,
		"cost_step": 160
	}
...
func _physics_process(delta: float) -> void:
	...
	update_player_radar_ping_timer(delta)
...
# ---------------- PLAYER RADAR PING ----------------
func update_player_radar_ping_timer(delta: float) -> void:
	if player_radar_ping_timer > 0.0:
		player_radar_ping_timer = max(player_radar_ping_timer - delta, 0.0)


func start_player_radar_ping() -> void:
	var bonus_seconds: float = 0.0

	if has_meta("upgrade_radar_bonus_seconds"):
		bonus_seconds = float(get_meta("upgrade_radar_bonus_seconds"))

	player_radar_ping_timer = PLAYER_RADAR_PING_BASE_TIME + bonus_seconds
	set_status_text_safe("Radar ping active for " + str(snapped(player_radar_ping_timer, 0.1)) + " seconds.")


func is_player_radar_ping_active() -> bool:
	return player_radar_ping_timer > 0.0
...
func create_title_screen() -> void:
	...
	title_shop_button = make_title_button("UPGRADE SHOP", Vector2(-165.0, 2.0), Vector2(330.0, 48.0))
	...
	title_brief_button = make_title_button("MISSION BRIEF", Vector2(-165.0, 58.0), Vector2(330.0, 46.0))
	title_brief_button.pressed.connect(toggle_title_brief)
	title_panel.add_child(title_brief_button)

	title_mode_button = make_title_button(get_mode_button_text(), Vector2(-165.0, 112.0), Vector2(330.0, 46.0))
	title_mode_button.pressed.connect(cycle_game_mode)
	title_panel.add_child(title_mode_button)

	title_quit_button = make_title_button("QUIT", Vector2(-165.0, 166.0), Vector2(330.0, 46.0))
...
func show_result_screen(result: String, extra_line: String = "") -> void:
	...
	if result_label:
		...
		result_label.text = (
			...
			"\nOpen the Upgrade Shop from the main menu."
		)
...
func build_shop_ui() -> void:
	...
	main_panel.offset_left = -430.0
	main_panel.offset_right = 430.0
	main_panel.offset_top = -310.0
	main_panel.offset_bottom = 310.0
	...
	outer_box.add_theme_constant_override("separation", 12)
	...
	add_upgrade_button(button_box, "radar")
...
func apply_battle_manager_upgrades() -> void:
	set_meta(
		"upgrade_radar_bonus_seconds",
		float(get_upgrade_level("radar")) * RADAR_BONUS_SECONDS_PER_LEVEL
	)
...
#    "health", "ammo", "stamina", "fuel", "radar", "money".
